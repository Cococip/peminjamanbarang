        ...
        <form action="<?= base_url('items/update/' . $item['id']) ?>" method="post">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="kode_barang" name="kode_barang" value="<?= esc(old('kode_barang') ?? $item['kode_barang']) ?>" class="<?= isset($errors['kode_barang']) ? 'is-invalid' : '' ?>">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nama_barang" name="nama_barang" value="<?= esc(old('nama_barang') ?? $item['nama_barang']) ?>" class="<?= isset($errors['nama_barang']) ? 'is-invalid' : '' ?>">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="jumlah" name="jumlah" min="1" value="<?= esc(old('jumlah') ?? $item['jumlah']) ?>" class="<?= isset($errors['jumlah']) ? 'is-invalid' : '' ?>">
                    <?php if (isset($errors['jumlah'])) : ?><div class="invalid-feedback"><?= esc($errors['jumlah']) ?></div><?php endif; ?>
                    <div class="form-hint">Total unit barang yang dimiliki. Jangan diubah lebih kecil dari jumlah yang sedang dipinjam.</div>
                </div>
                <div class="form-group">
                    ...
                    <select id="kondisi" name="kondisi" class="<?= isset($errors['kondisi']) ? 'is-invalid' : '' ?>">
                        <?php foreach (['Baik', 'Rusak Ringan', 'Rusak'] as $opt) : ?>
                            <option value="<?= $opt ?>" <?= (old('kondisi') ?? $item['kondisi']) === $opt ? 'selected' : '' ?>><?= $opt ?></option>
                        <?php endforeach; ?>
                    </select>
